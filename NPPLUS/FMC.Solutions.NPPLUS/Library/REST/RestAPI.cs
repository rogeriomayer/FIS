...
public class RestAPI
{
    ...
    public static Task<Resp> GetAsync<Resp>(string api, string routeMethod, IDictionary<string, string> headers, IDictionary<string, string> cookies, IDictionary<string, object> param = null, string accessToken = "")
    {
        var tcs = new TaskCompletionSource<Resp>();
        ...
        try
        {
            ...
            client.GetAsync(request, (response, handle) =>
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                    if (headers.Keys.Contains("Content-Type") && headers.Values.Contains("application/xml"))
                        tcs.SetResult(XmlHelper.GetObjectFromXML<Resp>(response.Content));
                    else
                        tcs.SetResult(JsonConvert.DeserializeObject<Resp>(response.Content));

                else
                    tcs.SetException(new Exception("Falha no metodo GET " + response.StatusCode.ToString() + " " + response.Content));
            });
    ...
    public static Resp Get<Resp>(string api, string routeMethod, IDictionary<string, string> headers, IDictionary<string, string> cookies, IDictionary<string, object> param = null, string accessToken = "")
    {
        try
        {
            ...
            if (response.StatusCode == HttpStatusCode.OK)
                if (headers.Keys.Contains("Content-Type") && headers.Values.Contains("application/xml"))
                    return XmlHelper.GetObjectFromXML<Resp>(response.Content);
                else
                    return JsonConvert.DeserializeObject<Resp>(response.Content);
