...
try
{
    ...
    while (DateTime.Now.DayOfWeek != DayOfWeek.Sunday)
    {
        ...
        var sended = new SendRcsBLL().GetSendToday();
        if (sended.ToList().Count() > 1000)
            break;

        IList<Person> listPerson = new PersonBLL().GetPersonSendRCS(dtIni).ToList();
        ...
        if (listPerson.Count > 0)
        {
            ...
            foreach (var person in listPerson.OrderBy(p => p.NrCNPJCPF).ToList())
            {
                ...
                if (idPerson != person.IdPerson)
                {
                    ...
                    var product = person.Product.Where(p => p.Lead.Where(l => l.DtInsert >= dtIni).Any()).FirstOrDefault();
