    ...
    public class ConsultaCpfCnpjController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                ...
                if (pessoa != null && pessoa.Cards.Any(x => x.Age > 0 && x.VlFull > 0))
                {
            ...
            try
            {
                ...
                if (_produto.NomeProduto == "IBI" && !string.IsNullOrEmpty(_produto.CodProduto))
                {
                    ...
                    if (pessoa != null)
                    {
                        ...
                        if (pessoa.Email != null && pessoa.Email.Count > 0)
                            email = pessoa.Email.FirstOrDefault();
        ...
        public IActionResult FormaDePagamento()
        {
            ...
            try
            {
                ...
                {
                    decimal vlEntrada = Convert.ToInt32(conta.VlFull / 4);
                    DateTime dtEntrada = DateTime.Today.AddDays(1);
                    ...
                    simulaParcelamento = new SimulaParcelamento()
                    {
                        ...
                        Entrada = vlEntrada < 40 ? Convert.ToDecimal(40).ToString("N2") : vlEntrada.ToString("N2"),
                        //Parcela = "0"
                    };
                ...
                if (Convert.ToDateTime(simulaParcelamento.DataEntrada) > DateTime.Today.AddDays(4))
                {
                    simulaParcelamento.DataEntrada = DateTime.Today.AddDays(4).ToString("dd/MM/yyyy");
                    message.Add("A data foi ajustada para a data de entrada máxima permitida " + simulaParcelamento.DataEntrada + " !");
                }
        ...
        public IActionResult TermosECondicoes(string totalParcel)
        {
            try
            {
                ...
                {
                    conta = pessoa.Cards.Where(p => p.CardNumber == _produto.CodProduto).FirstOrDefault();
                    ...
                    var statusLead = new StatusLead()
                    {
                        IdLead = conta.IdLead,
                        IdStatus = 4,
                        DsDescription = String.Format("Promessa de pagamento via portal para o dia {0} no valor de R${1}.", termo.DateEntranceParcel.ToString("dd/MM/yyyy"), termo.ValueEntrance.ToString("N2")),
                        DtInsert = DateTime.Now,
                        IdUserLogin = 1,
                        Promisse = new List<Promisse>()
                                {
                                        new Promisse
                                        {
                                            DtPromisse = termo.DateEntranceParcel,
                                            VlPromisse = termo.ValueEntrance,
                                            IdPromisseType = Convert.ToByte(1),
                                            NrParcel = Convert.ToByte(0) ,
                                            VlParcel = decimal.Zero,
                                            DtInsert=DateTime.Now
                                        }
                                }
                    };
                    
                    FisAPI.PostStatusLead(statusLead, pessoa.CPF, pessoa.Phones.FirstOrDefault().NrPhone, "");
                    */
        ...
        public IActionResult Finalizar()
        {
            try
            {
                string titulo = "Negociação Finalizada";
                ConsultaCpfCnpj model = cache.Get<ConsultaCpfCnpj>("model");
                if (model == null)
                    return RedirectToAction("SessaoExpirada", "ConsultaCpfCnpj");
                ...
                if (_produto != null && _produto.NomeProduto == "IBI")
                {
                    ...
                    CardResponse conta = pessoa.Cards.Where(p => p.Account.Contains(_produto.CodProduto)).FirstOrDefault();
                    ...
                    try
                    {
                        var billetRequest = new BilletRequest()
                        {
                            ...
                            NrParcel = agreement.NrParcel,
                            IdPromisse = 0,
                            CdAgreementRecupera = "",
                            VlBillet = agreement.VlEntrace,
                            DtBillet = agreement.DtEntrace,
                            CPF = pessoa.CPF
                        };
                        ...
                        if (boleto != null)
                        {
                            ...
                            boletoGerado = boleto;
                            linhaDigitavel = boleto.Line.Replace("  ", "-").Replace(" ", "").Replace("-", " ");
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (boleto == null)
                    {
                        var linha = FisAPI.GetLinhaDigitavelFatura(conta.AccountNumber, "");
                        if (linha != null && linha.responseData != null && !string.IsNullOrEmpty(linha.responseData.linhaDigitavel))
                        {
                            cache.Remove("linhaDigitavel");
                            linhaDigitavel = linha.responseData.linhaDigitavel;
                            cache.AddCache("linhaDigitavel", linhaDigitavel);
                        }
                        else
                        {
                ...
                IList<object> data = new List<object> { model, linhaDigitavel, cache.Get<List<Product>>("Product"), boletoGerado, pessoa, titulo };
        ...
        public IActionResult Finalizar(CardResponse conta = null)
        {
            try
            {
                ...
                DateTime dtEntrace = DateTime.Today.AddDays(1);
                string cpfCnpj = Regex.Replace(model.CpfCnpj, @"[^\d]", "");
...

        {
            try
            {
                ...
                PersonResponse pessoa = cache.Get<PersonResponse>("Pessoa");
                Produto _produto = cache.Get<Produto>("produto");
                CardResponse conta = pessoa.Cards.Where(p => p.Account == _produto.CodProduto).FirstOrDefault();
                if (string.IsNullOrEmpty(_contextAccessor.HttpContext.Session.GetString("_sID")))
                {
                    return RedirectToAction("Index", "Home");
                }

                //IbiService.Agreement wsAgreement = cache.Get<IbiService.Agreement>("Agreement");
                //if (wsAgreement != null)
                //{
                //    IbiService.URAClient client = new IbiService.URAClient();
                //    client.SetBilletAsync(new IbiService.Billet { DtInsert = DateTime.Now, IdProduct = wsAgreement.IdProduct }).GetAwaiter().GetResult();
                //}

                byte[] pdf = null;

                if (boleto != null)
                    pdf = FisAPI.GetBilletPDF(pessoa.CPF, boleto.CdAgreementRecupera, boleto.DtBillet, 1, String.Empty);
                else

                    pdf = FisAPI.GetFaturaPDF(pessoa.CPF, conta.AccountNumber, "PF", DateTime.Today.AddDays((conta.Age + 60) * -1), DateTime.Today, "");
                                ...
                                if (pessoa != null)
                                {
                                    ...
                                    if (conta != null)
                                    {
                                        ...
                                        var billetRequest = new SendSMSRequest
                                        {
                                            idProduct = agreement.IdProduct,
                                            cpf = cpf,
                                            codBillet = boleto.CdAgreementRecupera,
                                    ...
                                    if (pessoa != null)
                                    {
                                        ...
                                        if (conta != null)
                                        {
                                            ...
                                            var billetRequest = new SendEmailRequest
                                            {
                                                ...
                                                codBillet = boleto != null ? boleto.CdAgreementRecupera : "",
                                            ...
                                            var billetResponse = FisAPI.SendBilletEmail(billetRequest, 0, "");
        ...
        public ActionResult TermosDoAcordo(string totalParcel)
        {
            try
            {
                ...
                if (_produto != null && _produto.NomeProduto == "IBI")
                {
                    ...
                    string html = "<div class=\"box-termos\">";
                    if (termo.Age > AppSettings.MaxPromisse)
                    {
                        //termo do acordo
                        html += "<p>";
                        ...
                        html += "<p>Após o aceite desta proposta a sua negociação será processada e a reabilitação de seu CPF junto aos órgãos de proteção ao crédito(SPC / SERASA) ocorrerá em até 5 dias úteis.</p>";
                        html += "<p>Caso tenha utilizado o cartão recentemente e as despesas realizadas ainda não estejam postadas na fatura, tais valores não estarão inclusos na composição desta renegociação e será cobrada à parte em sua fatura.</p>";
                        html += "<p>Caso tenha formalizado um acordo referente a esta dívida em alguma assessoria nos últimos 3 dias, automaticamente o acordo efetivado agora neste canal será cancelado.</p>";
                        html += "<p>Se houver dúvidas, por favor, entre em contato com a nossa Central de Atendimento:</p>";
                        //pegar telefone da Afinz
                        html += "<p> 4090-1730(principais capitais) ou 0800 773 6660(demais regiões).</p>";
                        html += "<p>" +
                        ...
                        html += "</p>" +
                            "<p>Será cobrado IOF em todas as parcelas e caso a fatura atual esteja em atraso será cobrado multa com a 1ª parcela.</p>" +
                        ...
                        html += termo.CETMensal.ToString("N4") + "% ao mês <b>" + termo.VlCETMensal.ToString("C2") + "</b><br/>" +
                         termo.CETAnual.ToString("N4") + "% ao ano <b>" + termo.VlCETAnual.ToString("C2") + "</b></p>";

                        html += "<p>APÓS A FORMALIZAÇÃO NÃO SERÁ POSSÍVEL CANCELAR<br/><br/>Agradecemos sua atenção. Tenha ";
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (isCaptchaValid)
                    {
                        ...
                        if (pessoa != null && pessoa.Cards.Any(x => x.Age > 0))
                        {
