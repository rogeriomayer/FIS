...
public class FisAPI
{
    ...
    */
    ...
    {
        return RestAPI.Get<ICollection<ProductSpecification>>(Url_FIS_API, "productspecification/" + idProductType, null);
    }
    ...
    public static AcordoCredz SetAgreementCredz(AgreementSimulateRequest agreementRequest)
    {
        var result = RestAPI.Post<AcordoCredz, AgreementSimulateRequest>(Url_FIS_API, "agreement/addCredz", agreementRequest, "");
        return result;
    }
    ...
    {
        var result = RestAPI.Post<string, SMSRequest>(Url_FIS_API, "sms/" + 3, smsRequest, token);
        return result;
    }

    public static ICollection<Discount> GetDiscount(int idProductType, string token)
    {
        return RestAPI.Get<ICollection<Discount>>(Url_FIS_API, "discount/" + idProductType.ToString(), null, token);
    }
    ...
    */
    ...
    public static StatusLeadResponse PostStatusLead(StatusLead statusLead, string cpf, string telefone, int idProductType, string token)
    {
        var result = RestAPI.Post<StatusLeadResponse, StatusLead>(Url_FIS_API, "status/" + cpf + "/" + telefone + "/" + idProductType, statusLead, token);
        return result;
    }
}
